...
#+begin_comment
- replace all mentions of Israel with Zionist entity, etc.
- mention the point of the article early on, but spend time in the introduction
talking about the situation
- at the end of the intro (and at the end of the article link to sameer project
& alaa's fundraiser)
- make sure to quote alareer
#+end_comment
...
damage in Gaza, but had trouble finding the focus to finish it.(FOOTNOTE: add
note about west bank and lebanon as well) Obtaining accurate damage assessments
has been, in the last 15 months of genocide, obviously impossible, with the
Zionist occupiers having closed Gaza off completely to foreign journalists. With
some Palestinians now able to return to what remains of their homes, we are
starting to obtain pictures of the complete and utter destruction of entire
neighborhoods --- pictures reminiscent of the total annihilation caused by
American atomic bombs dropped on Japan. (CITE: pictures of devastation,
statistics about total tonnage of bombs dropped, finding dead buried in the
rubble) The human cost is, of course, unspeakable (FOOTNOTE + CITE AirWars
report https://gaza-patterns-harm.airwars.org/ and Lancet study).
...
sensing technique known as SAR interferometry, or InSAR, for short. (CITE:
https://www.npr.org/2023/11/16/1212889717/satellite-images-us-israel-gaza
https://www.space.commerce.gov/new-limits-on-satellite-imaging-of-israel/
https://www.bbc.com/news/57102499) Appropriately enough, InSAR is often used to
track large scale environmental changes (land subsidence, volcanic uplift,
deforestation and flood monitoring, etc. CITE:
https://sentiwiki.copernicus.eu/web/s1-applications) in addition to structural
damage. As Andreas Malm reminds us: the destruction of Palestine is the
destruction of the Earth (CITE:
...
example, in the paper [CITE paper]. Maps constructed using InSAR are used in the
media [CITE conflict damage citations] and, to my surprise, surprisingly
...
[[file:plots/insar-gaza/plot_20250125_012726_4802112.png]]
...
im = change.band_data.plot(ax=ax, vmin=-0.15, vmax=0, levels=5, cmap="RdBu")
...
#+RESULTS:
:results:
[[file:plots/insar-gaza/plot_20250125_013308_7008304.png]]
...
- this is one approach to tracking damage. can cross-check against local
  reporting and other satellite imagery
- can use this in the future as well. things likely become more and more
  complicated, though. can accumulate the results of multiple ccds? look for
  more papers over time by the same authors
- one benefit of a programmatic approach like this is that it is then quite
  straightforward to compile statistics. like the paper used spatial correlation
  against known humanitarian data to show that the zionist attacks tended to
  overlap with schools, etc.
- why do this exercise? who can be an intellectual (gramsci)? usage of
  technology that is used by those in power to surveil us (loewenstein, weizman,
  ...). an opportunity to learn the tools but to use it against them
